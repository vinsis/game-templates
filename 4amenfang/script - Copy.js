	...
	*/	
		...
		function checkForCollision(movingImage,direction){
			...
			for(image in game.bgImages){
				//if(game.bgImages[image].id.indexOf('Out')>=0){
					//continue;
				//}
				...
				if(direction=='left' && game.bgImages[image].guestId != movingImage.id){
					if(((mx1>sx1 && mx1<=sx2) && (mx2>sx2)) && ((my1>=sy1 && my1 <= sy2) || (my2>=sy1 && my2 <= sy2))){
						//console.log('left collision detected with block with guestID' + game.bgImages[image].guestId);
						mx1 = sx2+1;
						movingImage.x = mx1;
					}
				}else
				if(direction=='right' && game.bgImages[image].guestId != movingImage.id){
					if(((mx2>=sx1 && mx2<sx2) && (mx1<sx1 )) && ((my1>=sy1 && my1 <= sy2) || (my2>=sy1 && my2 <= sy2))){
						//console.log('right collision detected with ' + game.bgImages[image].guestId);
						mx2 = sx1-1;
						movingImage.x = mx2 - movingImage.w;
					}
				}else
				if(direction=='up' && game.bgImages[image].guestId != movingImage.id){
					if(((my1>=sy1 && my1<=sy2) && (my2>sy2 )) && ((mx1>=sx1 && mx1 <= sx2) || (mx2>=sx1 && mx2 <= sx2))){
						//console.log('up collision detected with ' + game.bgImages[image].guestId);
						my1 = sy2+1;
						movingImage.y = my1;
					}				
				}else
				if(direction=='down' && game.bgImages[image].guestId != movingImage.id){
					if(((my2>=sy1 && my2<=sy2) && (my1<sy1 )) && ((mx1>=sx1 && mx1 <= sx2) || (mx2>=sx1 && mx2 <= sx2))){
						//console.log('down collision detected with ' + game.bgImages[image].guestId);
						my2 = sy1-1;
