(function(){
	...
	*/	
		...
		function loadImageFromArrToArr(imageArr,targetArr){
			...
			for(i in imageArr){
				var image = new Image;
				image.src = imagePath + imageArr[i] + '.png';
				targetArr.push({
					pic: image,
					id: imageArr[i],
					guestId: -1,
		...
		function loadImageToArr(imageName,imageArr,loadedImages,reqdImages){
			...
			image.src = imagePath+imageName+'.png';
			var i = reqdImages;
		...
		function checkIfGameComplete(){
			...
			game.bgContext.font = "bold 80px monaco";
			game.bgContext.fillText('Game Over!', game.width/2-80,game.height/2-50);
			game.speed = 1;
			return true;
		}
			...
			for(parent in game.mobileImagesLocs){
				for(locs=0; locs<game.mobileImagesLocs[parent].length; locs++){
					...
					if(direction == 'left'){
							...
							console.log('slicing location from locs array ' + sx1 + ',' + sy1);
							game.mobileImagesLocs[parent].splice(locs-1,2);
							game.charContext.clearRect(sx1,sy1,game.mobileImages[parent].w,game.mobileImages[parent].h);
						}
					}
		...
		function drawGuestsInBlock(blockId,location){
			...
			if(location == 'left'){
				...
				writeTextOnCanvas(game.dict[game.parentImages[parent].id],game.bgContext,xLoc,yLoc-30);
			...
			if(location == 'right'){
				...
				yLoc = game.bgImages[j].y + 50;	//hardcoded
				writeTextOnCanvas(game.dict[game.parentImages[parent].id],game.bgContext,xLoc-15,yLoc-30);
				for(i in game.guestBlocks[parent]){
					//var temp = game.guestBlocks[parent][i];
			...
			if(location == 'top'){
				...
				writeTextOnCanvas(game.dict[game.parentImages[parent].id],game.bgContext,xLoc-70,yLoc+15);
				for(i in game.guestBlocks[parent]){
					game.mobileImagesLocs[game.guestBlocks[parent][i]].push(xLoc);
					game.mobileImagesLocs[game.guestBlocks[parent][i]].push(yLoc);					
					renderImage(game.mobileImages[game.guestBlocks[parent][i]],xLoc,yLoc,game.charContext);
			...
			if(location == 'bottom'){
				...
				writeTextOnCanvas(game.dict[game.parentImages[parent].id],game.bgContext,xLoc-60,yLoc+25);
				for(i in game.guestBlocks[parent]){
					game.mobileImagesLocs[game.guestBlocks[parent][i]].push(xLoc);
					game.mobileImagesLocs[game.guestBlocks[parent][i]].push(yLoc);					
					renderImage(game.mobileImages[game.guestBlocks[parent][i]],xLoc,yLoc,game.charContext);
