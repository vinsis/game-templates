(function(){
	$(document).ready(
		function(){
			...
			g.colsUsed = {};
			g.charsInTest = ['shang4','on top','above','zhi3','to stop','to prohibit','zheng4','main','positive'];
			g.charToCreate = g.charsInTest[Math.floor(Math.random()*6)]
			$('#charToCreate').text(g.charToCreate);
			
			
			var zheng4Grids = [10,11,12,14,15,16];
			for(i in zheng4Grids){
				//console.log(i);
				g.gridArray[zheng4Grids[i]]='zheng4';
			}
			var shang4Grids = [13,22,31,40,41,42,49,58,64,65,66,67,68,69,70];
